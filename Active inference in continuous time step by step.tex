...
In particular, a brain state $\mu$ is updated between two sequential steps $t$ and $t+1$ as
\begin{equation}
\mu^{t+1} = \mu^{t} - k \cdot \nabla_{\mu} F= \mu^{t} - k \cdot \nabla_{\mu} L(\tilde{\mu}, \tilde{s})
\end{equation}
